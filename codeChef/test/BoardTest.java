...
public class BoardTest {
	...
	public void testNegativeCoordinates() {
		...
		assertEquals("Invalid cell-coordinates: [-1, 0].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			board.state(0, -1);
		});
		assertEquals("Invalid cell-coordinates: [0, -1].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			board.state(-1, -1);
		});
		assertEquals("Invalid cell-coordinates: [-1, -1].", exception.getMessage());
	}
	...
	public void testLargeCoordinates() {
		...
		assertEquals("Invalid cell-coordinates: [" + height + ", 0].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			board.state(0, width);
		});
		assertEquals("Invalid cell-coordinates: [0, " + width + "].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			board.state(height, width);
		});
		assertEquals("Invalid cell-coordinates: [" + height + ", " + width + "].", exception.getMessage());
	}
	...
	public void testInvalidDimensions() {
		...
		assertEquals("Boards can only have non-zero positive dimensions; can't use [-1, -1].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			this.board = new Board(10, -1);
		});
		assertEquals("Boards can only have non-zero positive dimensions; can't use [10, -1].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			this.board = new Board(-1, 10);
		});
		assertEquals("Boards can only have non-zero positive dimensions; can't use [-1, 10].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			this.board = new Board(0);
		});
		assertEquals("Boards can only have non-zero positive dimensions; can't use [0, 0].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			this.board = new Board(10, 0);
		});
		assertEquals("Boards can only have non-zero positive dimensions; can't use [10, 0].", exception.getMessage());

		exception = assertThrows(IllegalArgumentException.class, () -> {
			this.board = new Board(0, 10);
		});
		assertEquals("Boards can only have non-zero positive dimensions; can't use [0, 10].", exception.getMessage());
	}
